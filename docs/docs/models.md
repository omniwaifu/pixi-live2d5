...
Destroying a model before it finishes loading cancels the creation: resources that arrive later are not attached
to the model (shared textures stay in the Pixi `Assets` cache), an internal model that was created but not yet
handed to the destroyed model is destroyed, no further `ready` or `load` event is emitted, and
`Live2DModel.from()` rejects (or `fromSync()` calls `onError`) with `Live2DModel was destroyed while loading.`
If an in-flight request is aborted first, its network error is reported instead.
