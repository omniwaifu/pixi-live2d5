...
本库用于渲染由应用开发者选择的模型资源。它不会对模型进行沙箱隔离，也不会限制模型引用资源的数量或大小、归档解压规模或网络访问策略。若应用允许加载第三方或用户提供的模型，应用必须在把资源交给 `Live2DModel` 之前，自行实施信任校验、下载策略、归档限制和资源预算。

可选的 `checkMocConsistency` 加载选项会请求 Cubism Core 检查 `.moc3` 文件的一致性。它只是额外的完整性检查，不是沙箱，也不能替代应用层控制。
...
原始项目致谢：[guansss/pixi-live2d-display](https://github.com/guansss/pixi-live2d-display)
